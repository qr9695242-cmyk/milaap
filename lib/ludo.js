...
export async function findOrCreateLudoMatch(userId, userName, userAvatar, betAmount) {
  ...
  const walletRef = doc(db, "users", userId, "ludoWallet", "balance");
  const waitingQuery = query(collection(db, "ludoMatches"), where("status", "==", "waiting"), where("betAmount", "==", Number(betAmount)));
  const waitingSnap = await getDocs(waitingQuery);
  const candidate = waitingSnap.docs.find(d => !(d.data().players || []).some(p => p.userId === userId));
